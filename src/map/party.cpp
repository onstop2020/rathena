 ...
 * @author Valaris
 **/
void party_exp_share(struct party_data* p, struct block_list* src, t_exp base_exp, t_exp job_exp, int zeny, int kp)
{
	...
	for (i = 0; i < c; i++) {
		...
		if (kp) { // Kill Points [Start]
			pc_setreg2(sd[i], "kp", pc_readreg2(sd[i], "kp") + kp);

			if (!pc_readreg2(sd[i], "is_kp_gain_no_display")) {
				char output[CHAT_SIZE_MAX];
				nullpo_retv(sd[i]);
				sprintf(output, "Kill Points +%d | Total: %d", kp, (int)pc_readreg2(sd[i], "kp"));
				clif_messagecolor(&sd[i]->bl, color_table[COLOR_KILL_POINTS], output, false, SELF);
			}
		}
	}
}
